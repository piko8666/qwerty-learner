...
const KeyboardWithBarCharts: FC<KeyboardWithBarChartsProps> = ({ data, title, suffix, name }) => {
  ...
  useEffect(() => {
    ...
    // 体验优化：为了精简柱状图排行，过滤掉无错误按键，且最多展示前 15 名
    const activeWrongData = myData.filter(item => item.value > 0).slice(0, 15)

    // 性能优化：不再执行彻底注销的 dispose()。否则会导致每次切暗黑模式时白屏闪烁且无 universalTransition 动画。
    ...
    const barOption = {
      ...
      yAxis: {
        type: 'value',
        minInterval: 1,
        axisLabel: { color: isOpenDarkMode ? '#fff' : '#000' }
      },
      xAxis: {
        type: 'category',
        axisLabel: {
        ...
        data: activeWrongData.map(function (item) {
          return item.name
        }),
      },
      animationDurationUpdate: 1000,
    ...
    // 针对暗黑/白天模式，使用纯 Option 更新机制进行驱动，激活形态补间动画
    chart.setOption(mapOption, true)
